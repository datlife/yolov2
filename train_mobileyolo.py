...
def _main_():
    ...
    yolov2     = MobileYolo(feature_extractor=darknet, num_anchors=N_ANCHORS, num_classes=N_CLASSES,
                            fine_grain_layer=['leaky_re_lu_13', 'leaky_re_lu_8'], dropout=0.3)
    ...
    yolov2.model.fit_generator(generator=train_data_gen,
                               steps_per_epoch=len*10,   #len(x_train) / BATCH_SIZE,
                               validation_data=val_data_gen,
                               validation_steps=len,   # int(len(x_train) * 0.1 / BATCH_SIZE),
...
def setup_debugger(yolov2):
    ...
    backup_model = LambdaCallback(on_epoch_end=lambda epoch, logs: yolov2.model.save_weights(
        BACK_UP_PATH + 'yolov2-epoch%s-loss%s.weights' % (epoch, str(logs.get('val_loss')))))
